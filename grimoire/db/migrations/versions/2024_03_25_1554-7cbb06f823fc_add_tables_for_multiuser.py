...
def downgrade() -> None:
    ...
    query = select(MessageNew)
    message_entries = session.scalars(query)
    old_message_entries = []
    for entry in message_entries:
        new_entry = {
            'id': entry.id,
            'chat_id': chats[entry.chat_id].external_id,
            'message_index': entry.message_index,
            'summary': entry.summary,
            'message': entry.message,
            'summary_tokens': entry.summary_tokens,
        ...
        old_message_entries.append(new_entry)

    session.execute(insert(MessageOld), old_message_entries)
    session.commit()
    op.drop_table('message_new')

    # Moving old ids into Knowledge table
    op.rename_table('knowledge', 'knowledge_new')
    ...
    query = select(KnowledgeNew)
    knowledge_entries = session.scalars(query)
    old_knowledge_entries = []
    ...
    for entry in knowledge_entries:
        ...
        old_knowledge_entries.append(new_entry)

    session.execute(insert(KnowledgeOld), old_knowledge_entries)
    session.commit()
    op.drop_table('knowledge_new')
